...
class ApiClient {
  ...
  async getSchemeMatches(
    ...
    query?: string,
    schemeType?: string
  ): Promise<{ schemes: Scheme[]; count: number }> {
    const params = new URLSearchParams();
    if (state) params.append('state', state);
    if (query) params.append('query', query);
    if (schemeType) params.append('scheme_type', schemeType);
    
    return this.request<{ schemes: Scheme[]; count: number }>(
      `/schemes/search?${params.toString()}`
    );
  }
  ...
  async checkHealth(): Promise<{ status: string; environment: string }> {
    ...
    const response = await fetch(`${rootUrl}/health`, { headers: { 'Accept': 'application/json' } });
